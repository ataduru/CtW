...
def server(request):
	...
	while 1:
		...
		for sock in ready_to_read:
			...
			if sock == server_socket:   #yeni client baglandigi zaman bu if calisir
				...
				for socket3 in SOCKET_LIST:
					if socket3 != server_socket and objects.empireIds.has_key(str(socket3.getpeername())) and objects.empireIds[str(socket3.getpeername())] == sayi:
						del objects.empireIds[str(socket3.getpeername())]
						SOCKET_LIST.remove(socket3)

				objects.empireIds[str(sockfd.getpeername())] = sayi
				if(sayi in secenekler):
					secenekler.remove(sayi)
				harita.y1 = objects.dicts[`sayi`].yuz_olcumu[0][0] - 20
				harita.y2 = objects.dicts[`sayi`].yuz_olcumu[1][0] + 20
				harita.x1 = objects.dicts[`sayi`].yuz_olcumu[0][1] - 20
				harita.x2 = objects.dicts[`sayi`].yuz_olcumu[1][1] + 20
				harita.baslangic_y1 = objects.dicts[`sayi`].yuz_olcumu[0][0] - 20
				harita.baslangic_y2 = objects.dicts[`sayi`].yuz_olcumu[1][0] + 20
				harita.baslangic_x1 = objects.dicts[`sayi`].yuz_olcumu[0][1] - 20
				harita.baslangic_x2 = objects.dicts[`sayi`].yuz_olcumu[1][1] + 20
				broadcastInd(server_socket, sockfd, harita.showMap(sayi)) 
				broadcastInd(server_socket, sockfd, (objects.dicts[`sayi`].ad+"\n") )
				broadcastInd(server_socket, sockfd, ("Coin:" + str(objects.dicts[`sayi`].coin) + "\n"))
				broadcastInd(server_socket, sockfd, ("Empire Defense:" + str(objects.dicts[`sayi`].sinir_savunmasi) + "\n"))
				broadcastInd(server_socket, sockfd, ("Wood:" + str(objects.dicts[`sayi`].tahta_miktari) + "Iron:" + str(objects.dicts[`sayi`].metal_miktari) + "\n"))
				broadcastInd(server_socket, sockfd, ("Attacking Soldier: " + str(objects.dicts[`sayi`].saldiri_askerleri.sayi) + "(" + str(objects.dicts[`sayi`].saldiri_askerleri.saldiri_gucu) + ") - Defensive Soldier: " + str(objects.dicts[`sayi`].savunma_askerleri.sayi) + "(" + str(objects.dicts[`sayi`].savunma_askerleri.savunma_gucu) + ")" + "\n"))

			 
			else:    #mevcut clientlar islem yaptigi zaman burasi calisir.
				try:
		    
					data = sock.recv(RECV_BUFFER)
					if data:
						sayi = objects.empireIds[str(sock.getpeername())]
						kul.empire_no = sayi
						...
						if(data.startswith('Attack ')):
							try:
								yeni = data.rstrip('\n')
								liste = yeni.split(' ')
								if(int(liste[1]) > 0 and int(liste[1]) < 257):
									kul.saldir(int(liste[1]))
									for socket2 in SOCKET_LIST:
										if socket2 != server_socket and objects.empireIds[str(socket2.getpeername())] == int(liste[1]):
											broadcastInd(server_socket, socket2, "Saldiriya ugradiniz\n")
								else:
									broadcastInd(server_socket, sock, "Invalid Input\n")
							except:
								broadcastInd(server_socket, sock, "Invalid Input\n")

						elif(data=="Improve Attack"):
							kul.saldiri_asker_gucu_arttir()

						elif(data=="Improve Defense"):
							...
							try:
								...
								liste = yeni.split(' ')
								if(int(liste[1]) > 0 and int(liste[1]) < 257):
									kul.hava_saldirisi_yap(int(liste[1]))
								else:
									broadcastInd(server_socket, sock, "Invalid Input\n")
							except:
								broadcastInd(server_socket, sock, "Invalid Input\n")
							...
							try:
								yeni = data.rstrip('\n')
								liste = yeni.split(' ')
								kul.saldiri_askeri_sayisi_arttir(int(liste[1]))
							except:
								broadcastInd(server_socket, sock, "Invalid Input\n")
							...
							try:
								yeni = data.rstrip('\n')
								liste = yeni.split(' ')
								kul.savunma_askeri_sayisi_arttir(int(liste[1]))
							except:
								broadcastInd(server_socket, sock, "Invalid Input\n")
						...
						elif(data=="Save and exit"):
							save()
							sys.exit(0)

						elif(data=="Mu"):
							objects.dicts[`sayi`].y1 -= 20
							objects.dicts[`sayi`].y2 -= 20
					
						elif(data=="Ml"):
							objects.dicts[`sayi`].x1 -= 20
							objects.dicts[`sayi`].x2 -= 20  
							...
							objects.dicts[`sayi`].x1 = objects.dicts[`sayi`].baslangic_x1			
							objects.dicts[`sayi`].x2 = objects.dicts[`sayi`].baslangic_x2
							objects.dicts[`sayi`].y1 = objects.dicts[`sayi`].baslangic_y1			
							objects.dicts[`sayi`].y2 = objects.dicts[`sayi`].baslangic_y2
...
def broadcastInd (server_socket, sock, message):
	for socket in SOCKET_LIST:
		if socket != server_socket and socket == sock :
		    try :
		        socket.send(message)
...
class Users:
	...
	if(empire_no != 0):
		harita.y1 = objects.dicts[`empire_no`].yuz_olcumu[0][0] - 20
		harita.y2 = objects.dicts[`empire_no`].yuz_olcumu[1][0] + 20
		harita.x1 = objects.dicts[`empire_no`].yuz_olcumu[0][1] - 20
		harita.x2 = objects.dicts[`empire_no`].yuz_olcumu[1][1] + 20
		harita.baslangic_y1 = objects.dicts[`empire_no`].yuz_olcumu[0][0] - 20
		harita.baslangic_y2 = objects.dicts[`empire_no`].yuz_olcumu[1][0] + 20
		harita.baslangic_x1 = objects.dicts[`empire_no`].yuz_olcumu[0][1] - 20
		harita.baslangic_x2 = objects.dicts[`empire_no`].yuz_olcumu[1][1] + 20
	...
	def savunma_asker_gucu_arttir(self):
		if(objects.dicts[`self.empire_no`].coin > 10000):
			objects.dicts[`self.empire_no`].savunma_askerleri.savunma_gucu += 1
			objects.dicts[`self.empire_no`].coin -= 10000

	def isci_sayisi_arttir(self):
		if(objects.dicts[`self.empire_no`].coin > 1000):
			objects.dicts[`self.empire_no`].tahta_ureten_isciler +=  int((objects.dicts[`self.empire_no`].tahta_ureten_isciler/2)*objects.dicts[`self.empire_no`].isci_okulu.saglamlik_durumu)
			objects.dicts[`self.empire_no`].metal_ureten_isciler +=  int((objects.dicts[`self.empire_no`].metal_ureten_isciler/2)*objects.dicts[`self.empire_no`].isci_okulu.saglamlik_durumu)
	...
	def saldir(self, empire_id):
		...
		if(((saldiran.sayi * (saldiran.saldiri_gucu + ekstra_atak))*2 + ((savunan.sayi * savunan.savunma_gucu) + objects.dicts[`empire_id`].sinir_savunmasi)*3) > 0):
			...
			if(toprak != toprakMiktari):
				...
				if(objects.dicts[`empire_id`].sinir_savunmasi - (objects.dicts[`empire_id`].sinir_savunmasi/10) > 0):
					objects.dicts[`empire_id`].sinir_savunmasi -= (objects.dicts[`empire_id`].sinir_savunmasi/10) 
...
def random_saldir(c, b):	
	...
	if(((saldiran.sayi * saldiran.saldiri_gucu)*2 + ((savunan.sayi * savunan.savunma_gucu) + objects.dicts[`b`].sinir_savunmasi)*3) > 0):
		...
		if(toprak != toprakMiktari):
			...
			if(objects.dicts[`b`].sinir_savunmasi - (objects.dicts[`b`].sinir_savunmasi/10) > 0):
				objects.dicts[`b`].sinir_savunmasi -= (objects.dicts[`b`].sinir_savunmasi/10)  
...
class Grid:	
	...
	def showMap(self, sayi):
		
		r = ""
		self.y1 = objects.dicts[`sayi`].y1
		self.y2 = objects.dicts[`sayi`].y2
		self.x1 = objects.dicts[`sayi`].x1
		self.x2 = objects.dicts[`sayi`].x2
...
class myClassC(Thread):
	...
	def run(self):
			...
			if(a!=b):
				random_saldir(a,b)
				for socket2 in SOCKET_LIST:
					if socket2 != SOCKET_LIST[0] and objects.empireIds[str(socket2.getpeername())] == b:
						broadcastInd(SOCKET_LIST[0], socket2, "Saldiriya ugradiniz\n")
			time.sleep(0.5)	
